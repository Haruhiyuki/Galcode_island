...
/// 中文任务 → 可选翻译 → 启动 Agent（`agent` 缺省 / `demo` / `claude-code` → Demo；`opencode` → 同目录 `opencode-cli`）。
#[tauri::command]
pub fn start_agent(
    app: AppHandle,
    ...
    match agent.as_deref() {
        Some("opencode") => manager::launch_opencode_agent(st, inner, cwd, user_input_zh),
        None | Some("demo") | Some("claude-code") => {
            manager::launch_demo_agent(st, inner, cwd, user_input_zh)
        }
        Some(other) => Err(format!(
            "暂不支持的 agent 类型: {}（请使用 opencode、demo 或 claude-code）",
            other
        )),
    }
}
...
pub fn update_llm_settings(
    ...
    crate::llm::client::update_global_settings(base_url, api_key, nickname, system_prompt, model);
    Ok(())
}
