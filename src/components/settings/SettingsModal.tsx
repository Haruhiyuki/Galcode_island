...
export function SettingsModal(): JSX.Element {
  ...
  const [localApiModel, setLocalApiModel] = React.useState(apiModel);
  ...
  React.useEffect(() => {
    if (isSettingsModalOpen) {
      ...
      setLocalApiModel(apiModel);
    }
  }, [isSettingsModalOpen, nickname, systemPrompt, apiKey, apiBaseUrl, apiModel]);
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    placeholder="留空：OpenAI 默认 gpt-4o-mini；URL 含 deepseek 则用 deepseek-v4-flash"
                    className="rounded-lg border border-black/5 bg-white/50 px-3 py-2 text-sm text-zinc-800 outline-none transition-all focus:border-sky-400/50 focus:bg-white/80 focus:ring-2 focus:ring-sky-400/15 dark:border-white/5 dark:bg-slate-800/50 dark:text-zinc-100 dark:focus:border-sky-400/40 dark:focus:bg-slate-800/70 dark:focus:ring-sky-400/10"
                  />
                  <p className="text-xs text-zinc-500 dark:text-zinc-400">
                    对接 DeepSeek 官方 API 时请填 <code className="rounded bg-black/5 px-1 dark:bg-white/10">deepseek-v4-flash</code> 或{" "}
                    <code className="rounded bg-black/5 px-1 dark:bg-white/10">deepseek-v4-pro</code>。
                  </p>
                </div>
              </div>
