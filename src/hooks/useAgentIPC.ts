...
export function useAgentIPC(): void {
  ...
  useEffect(() => {
    ...
    /** 允许「尚无 sessionId」时的首批事件：invoke 返回前后台可能已 emit，避免全部被丢弃。 */
    const forSession = (sid: string | undefined, fn: () => void) => {
      if (!sid) return;
      const current = sessionRef.current;
      if (current != null && sid !== current) return;
      fn();
    };
    ...
    const run = async () => {
      ...
      unsubs.push(
        await listen<SummaryReadyPayload>("agent://summary-ready", (e) => {
          const p = e.payload;
          forSession(p?.sessionId, () => {
            const store = storeRef.current;
            store.setMode(p.mode ?? "complete");
